...
| `POST /api/logs`                    | `{ companionId or companionIds, type, subtypes?, notes?, durationMinutes?, loggedAt? }` | Log a daily event. `type` is one of `walk`, `meal`, `bathroom`, `treat`, `play`, `grooming`, `other`. `subtypes` is an optional array of kinds valid for that type (e.g. `["pee"]` for `bathroom`, `["leash"]` for `walk`; the full list per type is in `/api/docs`); a value that isn't valid for the type is rejected with `400`. Returns `201 { ids, eventGroupId }`. |
